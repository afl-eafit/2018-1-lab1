...
To be used in CM0081 2018-1 lab 1. Defines NFAs and functions to create them
-}
module NFA where
...
-- | Represents an NFA as a 5-tuple
data NFA s a = NFA
...
-- | Adds transitions with of a state and symbol to an NFA
trans :: (Ord a, Ord s)
      => (s, a, Set.Set s)
      -- ^ Triplet with he state where the transition starts, the symbol of the
...
trans (q1, a, q2) nfa@(NFA q0 f q s tf)
  ...
  | otherwise       = NFA q0 f (Set.insert q1 (Set.union q2 q)) (Set.insert a s)
                        (insert q1 a q2 tf)
  where
    -- | Inserts (q1, a, q2) into the delta function of an NFA
...
-- | Adds a set of acceptance states to an NFA
accept :: (Ord a, Ord s)
       => Set.Set s
